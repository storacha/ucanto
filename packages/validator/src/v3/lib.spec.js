...
test.only("capability selects matches 2", assert => {
  const read = capability({
    can: "file/read",
    with: URI({ protocol: "file:" }),
    derives: (claimed, delegated) => {
...
test.only("derived capability chain 2", assert => {
  const verify = capability({
    can: "account/verify",
    with: URI({ protocol: "mailto:" }),
    derives: (claimed, delegated) => {
...
test.only("capability amplification 2", assert => {
  const read = capability({
    can: "file/read",
    with: URI({ protocol: "file:" }),
    derives: (claimed, delegated) =>
...
test.only("capability or combinator 2", assert => {
  const read = capability({
    can: "file/read",
    with: URI({ protocol: "file:" }),
    derives: (claimed, delegated) =>
